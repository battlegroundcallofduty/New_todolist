...
Python 3.13, uv, SQLite (aiosqlite), FastAPI, Vanilla JS, Bootstrap 5 CDN.
Linter: 없음 (소규모 프로젝트). CI: GitHub Actions (test + smoke).

## Documents
- `docs/AGENT_PROMPT.md` — 전체 빌드 플레이북. 수동 로드: "AGENT_PROMPT.md를 읽고 Phase N부터 실행해"
- `docs/STRATEGY.md` — 설계 원칙 + 하네스 방어 (이 파일에서 @참조로 자동 로드)
- `docs/SESSION_LOG.md` — 세션별 작업 기록. 수동 로드: "SESSION_LOG.md를 읽어"
...
단독 테스트: uv run pytest tests/test_todos.py::test_name -v
```
...
New todos get `position = COALESCE(MAX(position), -1) + 1`.

Frontend: `app.js`가 `/api/todos/`와 fetch로 통신. SPA — 페이지 새로고침 금지.
...
## SESSION_LOG 갱신
커밋 전에 해당 세션 작업을 `docs/SESSION_LOG.md`에 append (append only, 이전 내용 수정 금지).
